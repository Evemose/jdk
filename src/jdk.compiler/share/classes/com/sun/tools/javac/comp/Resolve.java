 ...
 ****************************************************************************/
    ...
    private Symbol findMethod(Env<AttrContext> env,
        ...
        if (bestSoFar.kind.isResolutionError()) {
            // if static top level methods will be valid some day, we should
            // also look in the toplevelScope
            var lookupScopes = new Scope[]{env.toplevel.namedImportScope, env.toplevel.starImportScope};
            bestSoFar = findExtMethodInScopes(
                    env, site, name,
                    argtypes, typeargtypes, bestSoFar,
                    allowBoxing, useVarargs, lookupScopes
            );
        }
    ...
    public Symbol findExtMethodInScopes(Env<AttrContext> env,
                                        Type site,
                                        Name name,
                                        List<Type> argtypes,
                                        List<Type> typeargtypes,
        ...
        for (Scope sc : scopes) {
            for (Symbol s : sc.getSymbolsByName(name)) {
                if (s.kind == MTH && (s.flags_field & EXTENSION) != 0) {
                    bestSoFar = selectBest(env, site, argtypes, typeargtypes, s,
                            bestSoFar, allowBoxing, useVarargs);
                    if (bestSoFar.kind.isValid()) {
                        return bestSoFar;
